...
def convert(audio_picker, model_picker, index_picker, index_rate, pitch, method):
    ...
    command = [
        ...
        str(pitch),
        "--input_path",
        f"audios/{audio_picker}",
        "--index_path",
        index_files[0],
        "--f0method",
        method,
        "--opt_path",
        f"audios/cli_output_{now}.wav",
        "--model_name",
        f"{model_picker}",
        "--index_rate",
    ...
    try:
        ...
        return {
            "choices": show_available("audios"),
            "__type__": "update",
            "value": f"cli_output_{now}.wav",
        }, f"audios/cli_output_{now}.wav"
    ...
    if model == "":
        try:
            ...
            return "You need to name your model. For example: My-Model", {
                "choices": show_available("assets/weights"),
                "__type__": "update",
            }
    ...
    try:
        ...
        for filename in os.listdir("./zips"):
            if filename.endswith(".zip"):
                ...
                shutil.copy2(
                    os.path.join("./zips/", filename), f"./assets/weights/{model}.pth"
                )
...
def refresh():
    return (
        {"choices": show_available("audios"), "__type__": "update"},
        {"choices": show_available("assets/weights", ".pth"), "__type__": "update"},
        {"choices": show_available("logs"), "__type__": "update"},
    )
...
with gr.Blocks() as app:
            ...
            with gr.Tabs():
                with gr.TabItem("1.Choose a voice model:"):
                    model_picker = gr.Dropdown(
                        label="Model: ",
                        choices=show_available("assets/weights", ".pth"),
                        value=show_available("assets/weights", ".pth")[0],
                        interactive=True,
                        allow_custom_value=True,
                    )
                    index_picker = gr.Dropdown(
                        label="Index:",
                        interactive=True,
                        choices=show_available("logs"),
                        value=show_available("logs")[0],
                        allow_custom_value=True,
                    )
                    model_picker.change(
                        fn=load_model,
                        inputs=[model_picker, index_picker],
                        outputs=[index_picker],
                    )
                with gr.TabItem("(Or download a model here)"):
                    with gr.Row():
                        url = gr.Textbox(
                            ...
                            value="",
                            placeholder="(i.e. https://huggingface.co/repo/model/resolve/main/model.zip)",
                        )
                    with gr.Row():
                        with gr.Column():
                            model_rename = gr.Textbox(
                        ...
                        with gr.Column():
                            ...
                            download_button.click(
                                ...
                                inputs=[url, model_rename],
                                outputs=[url, model_picker],
                            )
                    with gr.Row():
                        selected_import = gr.Dropdown(
                        ...
                        import_model.click(
                            fn=import_from_name,
                            inputs=[selected_import],
                            outputs=[selected_import, model_picker],
                        )
                with gr.TabItem("Advanced"):
                    ...
                    pitch = gr.Slider(
                        label="Pitch (-12 lowers it an octave, 0 keeps the original pitch, 12 lifts it an octave): ",
                        minimum=-12,
                        maximum=12,
                        step=1,
                        value=0,
                    ...
                    method = gr.Dropdown(
                        label="Method:", choices=["rmvpe", "pm"], value="rmvpe"
                    )
    ...
    with gr.Row():
            ...
            with gr.TabItem("2.Choose an audio file:"):
                ...
                try:
                    recorder.stop_recording(
                        upload_file, inputs=[recorder], outputs=[audio_picker]
                    )
                except:
                    recorder.upload(
                        upload_file, inputs=[recorder], outputs=[audio_picker]
        ...
        audio_refresher.click(
            fn=refresh, inputs=[], outputs=[audio_picker, model_picker, index_picker]
        )
    ...
    with gr.Row():
        ...
        audio_picker.change(
            fn=update_audio_player, inputs=[audio_picker], outputs=[audio_player]
        )
        convert_button.click(
            convert, inputs=inputs, outputs=[audio_picker, audio_player]
        )
